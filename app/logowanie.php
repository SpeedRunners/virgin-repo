...
session_start();
	...
	else
	{
		...
		sprintf("SELECT * FROM logowanie WHERE nick='%s' ",
		mysqli_real_escape_string($baza,$login))))
		{
			...
			if($ilu_us > 0)
			{
				$tablicadanych = $rezultat->fetch_assoc();
				if( password_verify($haslo,$tablicadanych['haslo']))
				{
				$_SESSION['zalogowany'] = true;
				$_SESSION['nazwagracza'] = $tablicadanych['nick'];
